    ...
    DDS_Boolean *dropped)
{
    ...
    * function and WindowUpdateSubscriber_deserialize_unsigned_long
    * to match your type.
    *
    * DDS_Long id = 0;
...
/*i
* \brief Helper function to filter an WindowUpdate sample
*
* \param[in]  sample       A WindowUpdate data sample to filter
...
WindowUpdateSubscriber_on_before_sample_commit(
    ...
    DDS_Boolean *dropped)
{
    WindowUpdate *hw_sample = (WindowUpdate *)sample; 

    (void)listener_data;
    (void)reader;
    (void)sample_info;

    WindowUpdateSubscriber_filter_sample(hw_sample, dropped);
...
static void
WindowUpdateSubscriber_on_subscription_matched(
    void *listener_data,
    DDS_DataReader *reader,
...
static void
WindowUpdateSubscriber_on_data_available(
    void *listener_data,
    DDS_DataReader * reader)
{
    WindowUpdateDataReader *hw_reader = WindowUpdateDataReader_narrow(reader);
    ...
    struct WindowUpdateSeq sample_seq = 
    DDS_SEQUENCE_INITIALIZER;
    ...
    retcode = WindowUpdateDataReader_take(
        hw_reader, 
        &sample_seq,
        &info_seq,
    ...
    for (i = 0; i < WindowUpdateSeq_get_length(&sample_seq); ++i)
    {
        sample_info = DDS_SampleInfoSeq_get_reference(&info_seq, i);

        if (sample_info->valid_data)
        {
            sample = WindowUpdateSeq_get_reference(&sample_seq, i);
    ...
    WindowUpdateDataReader_return_loan(hw_reader, &sample_seq, &info_seq);

    done:
    #ifndef RTI_CERT
    WindowUpdateSeq_finalize(&sample_seq);
    ...
    DDS_Long count)
{
    ...
    */
    ...
    dr_listener.on_data_available = WindowUpdateSubscriber_on_data_available;
